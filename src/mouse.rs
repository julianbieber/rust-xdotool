...
impl XServer {
    ...
    /// println!("{}", output.status);
    /// ```
    pub fn move_mouse(&self, x: u16, y: u16, options: OptionVec<MouseMoveOption>) -> Output {
    ...
    /// ]);
    /// ```
    pub fn click(&self, button: Button, options: OptionVec<ClickOption>) -> Output {
        let c = Command::Mouse(sub_commands::Mouse::Click(options));
        self.run(c, &button.to_string())
    }
    ...
    /// - `ClickOption::Window(String)` Specify a window to send a click to.
    /// - `ClickOption::ClearModifiers`
    pub fn click_down(&self, button: Button, options: OptionVec<ClickOption>) -> Output {
    ...
    /// - `ClickOption::Window(String)` Specify a window to send a click to.
    /// - `ClickOption::ClearModifiers`
    pub fn click_up(&self, button: Button, options: OptionVec<ClickOption>) -> Output {
    ...
    /// # Ok::<(), std::string::FromUtf8Error>(())
    /// ```
    pub fn get_mouse_location(&self) -> Output {
    ...
    /// # Options
    ///
    /// - `BehaveScreenEdgeOption::Delay(u32)` Delay in milliseconds before self.running the command. This allows require a given edge or corner to be held for a short period before your command will self.run. If you leave the corner before the delay expires then the time will reset.
    /// - `BehaveScreenEdgeOption::Quiesce(u32)`  Delay in milliseconds before the next command will self.run. This helps prevent accidentally running your command extra times; if you have a very short --delay (like the default of 0).
    ///
    /// # Examples
    ///
    ...
    ///
    /// ```no_self.run
    /// # use xdotool::{options, sub_commands, Command};
